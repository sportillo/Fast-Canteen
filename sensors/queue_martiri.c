...
PERIODIC_RESOURCE(GetStatus, METHOD_GET, "status", "title=\"Queue\";obs", 5*CLOCK_SECOND);
...
void GetStatus_periodic_handler(resource_t *r)
{
  ...
  coap_set_payload(notification, content, snprintf(content, sizeof(content), "{\"bn\":\"martiri\",\"e\":[{\"n\":\"queue\",\"id\":\"%d\",\"bv\":\"%d\"}]}", id, status));
  printf("%s\n", content);
